package server

import (
	"context"
	"fmt"
	"net"
	"smol/smolhabittracker/api"
	"smol/smolhabittracker/internal/habit"
	"strconv"
	"time"

	"golang.org/x/sync/errgroup"
	"google.golang.org/grpc"
	"google.golang.org/grpc/reflection"
)

type Server struct {
	api.UnimplementedHabitsServer
	db  Repository
	lgr Logger
}

type Repository interface {
	Add(ctx context.Context, habit habit.Habit) error
	List(ctx context.Context, id habit.ID) (habit.Habit, error)
	ListAll(ctx context.Context) ([]habit.Habit, error)
	AddTick(ctx context.Context, id habit.ID, t time.Time) error
	ListAllTicks(ctx context.Context, id habit.ID) ([]time.Time, error)
	ListWeeklyTicks(ctx context.Context, id habit.ID, t time.Time) ([]time.Time, error)
}

func New(repo Repository, lgr Logger) *Server {
	return &Server{
		db:  repo,
		lgr: lgr,
	}
}

func (s *Server) ListenAndServe(ctx context.Context, port int) error {
	const addr = "127.0.0.1"

	listener, err := net.Listen("tcp", net.JoinHostPort(addr, strconv.Itoa(port)))
	if err != nil {
		return fmt.Errorf("unable to listen to tcp port %d: %w", port, err)
	}

	grpcServer := s.registerGRPCServer()
	s.lgr.Logf("gRPC server started and listening on port %d\n", port)

	errChan := make(chan error)
	g := errgroup.Group{}
	defer func() {
		err := g.Wait()
		if err != nil {
			errChan <- fmt.Errorf("error while serving: %w", err)
		}
		close(errChan)
	}()

	g.Go(func() error {
		err := grpcServer.Serve(listener)
		if err != nil {
			s.lgr.Logf("error while serving gRPC: %s", err)

			return fmt.Errorf("gRPC server error: %w", err)
		}

		return nil
	})

	select {
	case <-ctx.Done():
		s.lgr.Logf("Shutting down grpc server: %s", ctx.Err())
	case err = <-errChan:
		s.lgr.Logf("unable to serve: %w", err)
	}

	grpcServer.GracefulStop()
	_ = listener.Close()

	return nil
}

func (s *Server) registerGRPCServer() *grpc.Server {
	grpcServer := grpc.NewServer(grpc.UnaryInterceptor(timerInterceptor(s.lgr)))
	api.RegisterHabitsServer(grpcServer, s)
	reflection.Register(grpcServer)
	return grpcServer
}

type Logger interface {
	Logf(format string, args ...any)
}
